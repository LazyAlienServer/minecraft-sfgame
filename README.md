...
`async` 同时开放所有点；`sync` 每局随机排列点位，一次只开放并显示一个点，切换时会提示“点位 A 已沦陷，请前往占领点位 B”并播放提示音，完成本轮全部点位后结算。点位 ID 支持单个字母，输入的大写字母会按同一小写 ID 保存。长方形使用两个角点，正方形使用管理员脚下中心与半径；未设置高度时覆盖全部高度。占点默认每 1 秒为每个已归属活动点增加 1 分。
